...
*   Declaration of local internal table
    DATA: lt_status_list  TYPE scpr_transp_entries,
          lt_field_descrs TYPE scpr_records.
...
*   Load user settings
    cl_scpr_settings=>load_user_settings(
      IMPORTING
        e_user_settings = DATA(ls_user_setting)
    ).

    IF ls_user_setting-urecdel = abap_false.

      ls_user_setting-urecdel = abap_true.

      cl_scpr_settings=>update_user_settings( ls_user_setting ).
